...
namespace godot {
    ...
    /**
     * SceneDataManager handles the GPU buffers for scene data such as spheres, triangles, vertices, and materials.
     */
    ...
    public:
        ...
        uint32_t get_sphere_count() const { return _frame_stats.sphere_count; }
        uint32_t get_triangle_count() const { return _frame_stats.triangle_count; }
        uint32_t get_vertex_count() const { return _frame_stats.vertex_count; }
        uint32_t get_material_count() const { return _frame_stats.material_count; }

    private:
        void _update_spheres_buffer(const TypedArray<Node>& all_nodes, const std::vector<uint32_t>& sphere_indices);
        void _update_triangles_buffer(const TypedArray<Node>& all_nodes, const std::vector<uint32_t>& triangle_mesh_indices);
