...
void PTBoundingVolumeHierarchy::build(
...
static inline Vector3 compute_centroid(
    const std::vector<PTVertex> &vertices,
    const PTTriangle &tri)
{
    return (vertices[tri.i0].position + 
            vertices[tri.i1].position + 
            vertices[tri.i2].position) / 3.0f;
}
    ...
    uint32_t end)
{
    ...
    // Sort by the chosen axis (single sort instead of separate function call)
...
void PTBoundingVolumeHierarchy::split(
    uint32_t nodeIndex, 
    const std::vector<PTVertex> &vertices, 
    std::vector<PTTriangle> &triangles, 
    uint32_t begin,
    ...
    uint32_t depth)
{
    ...
    if (count <= 10 || depth >= 20) {
        ...
        nodes[nodeIndex].aabb = compute_aabb(vertices, triangles, begin, end);
        return;
    }
    ...
    split(left_node_index, vertices, triangles, begin, split_index, depth + 1);
    split(right_node_index, vertices, triangles, split_index, end, depth + 1);

    // Set up current node as internal node
    // Access node data after recursive calls to avoid reference invalidation
    ...
    nodes[nodeIndex].aabb = nodes[left_node_index].aabb.merge(nodes[right_node_index].aabb);
