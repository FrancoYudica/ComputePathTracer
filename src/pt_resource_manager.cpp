...
{
    ClassDB::bind_method(D_METHOD("initialize", "rd", "camera"), &PTResourceManager::initialize);
    ClassDB::bind_method(D_METHOD("resize", "width", "height"), &PTResourceManager::resize);
    ClassDB::bind_method(D_METHOD("get_output_texture"), &PTResourceManager::get_output_texture);
    ClassDB::bind_method(D_METHOD("get_accumulation_texture"), &PTResourceManager::get_accumulation_texture);
    ClassDB::bind_method(D_METHOD("get_skybox_texture"), &PTResourceManager::get_skybox_texture);
    ClassDB::bind_method(D_METHOD("get_pipeline"), &PTResourceManager::get_pipeline);
    ClassDB::bind_method(D_METHOD("get_shader"), &PTResourceManager::get_shader);
    ...
    ClassDB::bind_method(D_METHOD("get_scene_uniform_set"), &PTResourceManager::get_scene_uniform_set);
}
...
PTResourceManager::~PTResourceManager()
{
    ...
    // _rd->free_rid(_skybox_texture); // TODO: Only free if it's the one created here
...
{
    // Texture format
    Ref<RDTextureFormat> texture_format = Ref<RDTextureFormat>(memnew(RDTextureFormat));
    ...
    // Texture view
    Ref<RDTextureView> texture_view = Ref<RDTextureView>(memnew(RDTextureView));
...
void PTResourceManager::initialize(
    RenderingDevice* rd, 
    Camera3D* camera, 
    String shader_path,
    uint32_t width, 
...
{
    // LDR output texture
    _output_texture = create_texture(_rd, width, height, RenderingDevice::DATA_FORMAT_R16G16B16A16_SFLOAT);

    // HDR accumulation texture
    _accumulation_texture = create_texture(_rd, width, height, RenderingDevice::DATA_FORMAT_R32G32B32A32_SFLOAT);
}
...
void PTResourceManager::_load_skybox_texture()
{
    ...
    if (!_skybox_texture.is_valid()) {
        ...
        _skybox_texture = create_texture(_rd, 1, 1, RenderingDevice::DATA_FORMAT_R8G8B8A8_UNORM);
    }
}
...
RID PTResourceManager::_get_camera_skybox_texture(Camera3D* camera)
{
    ...
    // Convert engine Texture RID to an RD texture RID understood by the RenderingDevice
    RID engine_tex_rid = cubemap->get_rid();
    RID rd_tex = RenderingServer::get_singleton()->texture_get_rd_texture(engine_tex_rid);
    return rd_tex;
}
...
void PTResourceManager::_create_uniforms()
{
    ...
    // Image uniforms
    {
        ...
        _output_image_uniform->set_uniform_type(RenderingDevice::UNIFORM_TYPE_IMAGE);
        _output_image_uniform->set_binding(0);
        _output_image_uniform->add_id(_output_texture);

        // Accumulation image
        _accumulation_image_uniform = Ref<RDUniform>(memnew(RDUniform));
        _accumulation_image_uniform->set_uniform_type(RenderingDevice::UNIFORM_TYPE_IMAGE);
        _accumulation_image_uniform->set_binding(1);
        _accumulation_image_uniform->add_id(_accumulation_texture);

        // Skybox image
        _skybox_image_uniform = Ref<RDUniform>(memnew(RDUniform));
        _skybox_image_uniform->set_uniform_type(RenderingDevice::UNIFORM_TYPE_SAMPLER_WITH_TEXTURE);
        _skybox_image_uniform->set_binding(2);

        // Skybox sampler
        Ref<RDSamplerState> sampler_state = Ref<RDSamplerState>(memnew(RDSamplerState));
    ...
    // Settings uniform
    {
        ...
        _settings_uniform->set_uniform_type(RenderingDevice::UNIFORM_TYPE_STORAGE_BUFFER);
    ...
    // Camera uniform
    {
        ...
        _camera_uniform->set_uniform_type(RenderingDevice::UNIFORM_TYPE_STORAGE_BUFFER);
    ...
    // Scene uniforms 
    {
        ...
        _scene_spheres_uniform->set_uniform_type(RenderingDevice::UNIFORM_TYPE_STORAGE_BUFFER);
        _scene_spheres_uniform->set_binding(0);

        _scene_triangles_uniform = Ref<RDUniform>(memnew(RDUniform));
        _scene_triangles_uniform->set_uniform_type(RenderingDevice::UNIFORM_TYPE_STORAGE_BUFFER);
        _scene_triangles_uniform->set_binding(1);

        _scene_vertex_uniform = Ref<RDUniform>(memnew(RDUniform));
        _scene_vertex_uniform->set_uniform_type(RenderingDevice::UNIFORM_TYPE_STORAGE_BUFFER);
...
void PTResourceManager::_create_storage_buffers()
{
    ...
    }
    {
        ...
        _scene_triangles_storage_buffer = _rd->storage_buffer_create(triangles_bytes.size(), triangles_bytes);
...
void PTResourceManager::_create_shader_and_pipeline(String shader_path)
{
    // Shader
    {
        ...
        if (shader_file.is_null()) {
            ERR_PRINT("Failed to load path tracer shader file. " + shader_path);
            return;
        }
        Ref<RDShaderSPIRV> spirv = shader_file->get_spirv();
        if (spirv.is_null()) {
            ERR_PRINT("Shader file does not contain SPIR-V bytecode: " + shader_path);
            return;
        }
