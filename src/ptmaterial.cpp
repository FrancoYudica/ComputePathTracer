...
{
	// Binds methods
    ClassDB::bind_method(D_METHOD("set_color", "color"), &PTMaterial::set_color);
    ClassDB::bind_method(D_METHOD("get_color"), &PTMaterial::get_color);

    ClassDB::bind_method(D_METHOD("set_metallic", "metallic"), &PTMaterial::set_metallic);
    ClassDB::bind_method(D_METHOD("get_metallic"), &PTMaterial::get_metallic);

    ClassDB::bind_method(D_METHOD("set_roughness", "roughness"), &PTMaterial::set_roughness);
    ClassDB::bind_method(D_METHOD("get_roughness"), &PTMaterial::get_roughness);

    ClassDB::bind_method(D_METHOD("set_refraction_index", "refraction_index"), &PTMaterial::set_refraction_index);
    ClassDB::bind_method(D_METHOD("get_refraction_index"), &PTMaterial::get_refraction_index);

    ClassDB::bind_method(D_METHOD("set_emission", "emission"), &PTMaterial::set_emission);
    ClassDB::bind_method(D_METHOD("get_emission"), &PTMaterial::get_emission);

    ClassDB::bind_method(D_METHOD("get_material_type"), &PTMaterial::get_material_type);
    ClassDB::bind_method(D_METHOD("set_material_type", "material_type"), &PTMaterial::set_material_type);

	// Add properties
	ADD_PROPERTY(PropertyInfo(Variant::INT, "material_type", PROPERTY_HINT_ENUM, "Lambertian,Metal,Dielectric,Emissive"), "set_material_type", "get_material_type");
	...
	ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "refraction_index", PROPERTY_HINT_RANGE, "0,3,0.01"), "set_refraction_index", "get_refraction_index");
	ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "emission", PROPERTY_HINT_RANGE, "0,100,0.01"), "set_emission", "get_emission");
...
void godot::PTMaterial::set_color(const Color &p_color)
{
  color = p_color;
}

godot::Color godot::PTMaterial::get_color() const
{
    return color;
}
...
float godot::PTMaterial::get_metallic() const
{
    return metallic;
}
...
float godot::PTMaterial::get_roughness() const
{
    return roughness;
}
...
float godot::PTMaterial::get_emission() const
{
    return emission;
}
...
{
    return "PTMaterial(type=" + String::num_int64(static_cast<int64_t>(material_type)) +
